...
function createBtnGroup() {
    ...
    infoBtn.innerHTML = '<i class="fa fa-refresh" aria-hidden="ture"></i>';
    let delBtn = createAnyElement("button", {class: "btn btn-danger", onclick: "delRow(this)"});
    delBtn.innerHTML ='<i class= "fa fa-trash" aria-hidden="ture"></i>';
...
function createUser(btn){
    ...
    fetch('http://localhost:3000/users', fetchOption).then(
        resp => resp.json(),
        err => console.error(err)
    ).then(
        data => startGetUsers()
